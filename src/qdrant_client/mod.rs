...
impl<I: Send + Sync + 'static + Clone + Interceptor> Qdrant<I> {
    ...
    ///
    ///# async fn connect() -> Result<(), qdrant_client::QdrantError> {
    /// let client = Qdrant::from_url_with_custom_interceptor(
    ///     "http://localhost:6334",
    ///     CustomInterceptor
    /// ).build()?;
    ///# Ok(())
    ///# }
    /// ```
    pub fn from_url_with_custom_interceptor(url: &str, interceptor: I) -> QdrantConfig<I> {
        QdrantConfig::<I>::from_url_with_custom_interceptor(url, interceptor)
    }
